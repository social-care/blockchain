...
IMAGE=$(docker images | grep "blockchain-0.0.1" | cut -d' ' -f1);
CONTAINER=$(docker ps -aq --filter "name=blockchain-0.0.1");
NETWOKCARD=$(composer card list | grep admin@blockchain);
